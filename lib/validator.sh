...
measure_and_rank() {
    ...
    # 1. Obtener IP real de referencia (la puedes seguir mostrando, aunque ya no la uses para comparar)
    local real_ip
    real_ip=$(dig @1.1.1.1 "$target" +short +tries=1 +timeout=2 \
        | grep -E '^([0-9]{1,3}\.){3}[0-9]{1,3}$' | tail -n1)
    ...
    if [ "$mode" = "direct" ]; then
        ...
        # Versión laxa: cualquier IP A válida y con Query time
        xargs -P "$threads" -I {} sh -c '
            target="$0"
            res_ip=$(dig @"{}" "$target" +short +tries=1 +timeout=1 \
                | grep -E "^([0-9]{1,3}\.){3}[0-9]{1,3}$" | tail -n1)

            ms=$(dig @"{}" "$target" +tries=1 +timeout=1 2>/dev/null \
    ...
    # --- CAMINO B: MODO FULL (Doble Paso) ---
        ...
        xargs -P "$threads" -I {} sh -c '
            ...
            if [ -n "$ms" ]; then
                echo "$ms {}"
                printf "\033[36m[·]\033[0m {} | ${ms}ms\n" > /dev/tty
            fi
        ' "$target" < "$filtered_list" > "$tmp_bench"
    fi

    # --- FINALIZACIÓN COMÚN (Ordenar y Limpiar) ---
    sort -n "$tmp_bench" | awk '{print $2}' \
        | { [[ "$limit" == "all" ]] && cat || head -n "$limit"; } > "$output"
