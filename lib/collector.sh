...
update_master_raw() {
  echo -e "\e[1;34m[➜]\e[0m Actualizando base RAW desde fuentes externas..."
  ...
  echo -e "\e[1;34m[➜]\e[0m Limpieza de duplicados en RAW..."
...
validate_raw_to_live() {
  local threads=$1
  echo -e "\e[1;34m[➜]\e[0m Validando RAW contra LIVE..."
  ...
  echo -e "\e[1;34m[➜]\e[0m Verificando resolvers vivos en RAW..."
  ...
  if [ -s "$LIVE_FILE" ]; then
    echo -e "\e[1;34m[➜]\e[0m Filtrando resolvers que ya están en LIVE..."
    # Extraer IPs de tmp_vivos que NO están en LIVE_FILE
    grep -F -v -f "$LIVE_FILE" "$tmp_vivos" > "$tmp_new_to_validate"
  else
    echo -e "\e[1;34m[➜]\e[0m No hay LIVE_FILE, utilizando todo RAW..."
    cat "$tmp_vivos" > "$tmp_new_to_validate"
  fi

  # 3. Validación con dnsvalidator
  echo -e "\e[1;34m[➜]\e[0m Validando resolvers con dnsvalidator..."
  dnsvalidator -tL "$tmp_new_to_validate" -threads "$threads" -o "$tmp_validated"
  
  # 4. Anexar validados a LIVE
  echo -e "\e[1;34m[➜]\e[0m Anexando resolvers validados a LIVE..."
